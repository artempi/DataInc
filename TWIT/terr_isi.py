...
def extract_tweet_ids(soup):
	...
	for tweet in tweets:
		if tweet.find('p','tweet-text'):
			...
			continue
		time = tweet.find('a', {'class': 'tweet-timestamp'})['title'].encode('UTF-8')
		#print(time)
...
def save_tweets(hashtags, n, group):
    ...
    for hashtag in hashtags:
        ...
        # write to file
        with open('save_terr_isi'+str(i)+'.p'.format(group,hashtag), 'ab') as f:
            pickle.dump(tweet_ids,f)



if __name__ == '__main__':
    hashtags = ['terrorism OR isis']
    save_tweets(hashtags, n=1000, group='set3')
    driver.quit()
